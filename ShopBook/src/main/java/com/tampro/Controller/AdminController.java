...
public class AdminController {
	...
	public String confirmOrder(ModelMap map,@PathVariable("id")int id) {		
		...
		for(CartItem cart : order.getList()) { // những items đã đặt của order ta duyệt
			cart.getBook().setQuantity(cart.getBook().getQuantity()-cart.getQuantity()); // lấy số lượng -  quantity items  set lại vào cart
			bookservice.updateBook(cart.getBook()); //update book
		}
	...
	public String category(ModelMap map) {	
		map.addAttribute("category", bookCategoryService.getAllBookCategory());
			
		return "category";
	}
	@GetMapping("/them-danh-muc") 
	public String categoryform(ModelMap map) {	
		map.addAttribute("category",new BookCategory());			
		return "formcategory";
	}
	@PostMapping("/add-category") 
	public String category(ModelMap map,@Valid @ModelAttribute("category") BookCategory bookCategory,BindingResult bindingResult) {
	...
	@GetMapping("/update-danh-muc/{id}") 
	public String category(ModelMap map,@PathVariable ("id") int id) {	
		map.addAttribute("category",bookCategoryService.getBookCategoryById(id));			
		return "updatecategory";
	}
	@PostMapping("/update-danh-muc") 
	public String categoryupdate(ModelMap map, @Valid @ModelAttribute("category") BookCategory category,BindingResult result) {	
		if(result.hasErrors()) {
			return "updatecategory";
		}
	...
	public String banner(ModelMap map) {	
			map.addAttribute("banner", bannerService.getAllBanner());
		return "banner";
	}
